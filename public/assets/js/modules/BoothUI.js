...
export class BoothUI {
  ...
  showLoadingOverlay(message = "Processing...") {
    ...
    if (!overlay) {
      ...
      overlay.style.cssText = "position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(15,23,42,0.6);display:flex;flex-direction:column;justify-content:center;align-items:center;z-index:99999;color:white;font-family:'Plus Jakarta Sans',sans-serif;backdrop-filter:blur(20px);-webkit-backdrop-filter:blur(20px);transition:all 0.3s ease;";
      overlay.innerHTML = "<div style=\"text-align:center;max-width:90%;width:420px;padding:35px;background:rgba(255,255,255,0.08);border-radius:24px;border:1px solid rgba(255,255,255,0.15);box-shadow:0 24px 60px rgba(0,0,0,0.4);backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px);\"><div class=\"spinner-container\" style=\"position:relative;width:90px;height:90px;margin:0 auto 25px;\"><div style=\"position:absolute;inset:0;border:4px dashed rgba(255,255,255,0.15);border-radius:50%;\"></div><div class=\"spinner\" style=\"position:absolute;inset:0;border:4px solid transparent;border-top:4px solid #a855f7;border-right:4px solid #eab308;border-radius:50%;animation:spin 1s linear infinite;\"></div></div><div class=\"loading-text\" style=\"font-size:1.4rem;font-weight:700;margin-bottom:15px;color:#fff;text-shadow:0 2px 8px rgba(0,0,0,0.3);\">" + message + "</div><div style=\"width:100%;height:8px;background:rgba(255,255,255,0.1);border-radius:10px;overflow:hidden;margin-bottom:15px;border:1px solid rgba(255,255,255,0.05);\"><div class=\"progress-bar\" style=\"width:0%;height:100%;background:linear-gradient(90deg,#a855f7,#eab308);transition:width 0.4s ease;border-radius:10px;\"></div></div><div class=\"loading-subtext\" style=\"font-size:0.9rem;color:rgba(255,255,255,0.75);min-height:22px;font-weight:500;\">Harap tunggu sebentar...</div></div><style>@keyframes spin{0%{transform:rotate(0deg);}100%{transform:rotate(360deg);}}</style>";
