...
export async function initPermissions(){
  ...
  if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
    try {
      result.cameraStream = await navigator.mediaDevices.getUserMedia({video:true,audio:false});
      console.log('[InitPermissions] Camera access granted');
    } catch(e) {
      console.warn('[InitPermissions] Primary camera request failed, trying fallback...', e);
      try {
        result.cameraStream = await navigator.mediaDevices.getUserMedia({video:{facingMode:"user"},audio:false});
    ...
    console.error('[InitPermissions] mediaDevices.getUserMedia not available (Secure Context required)');
  }
